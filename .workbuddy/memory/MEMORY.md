...
- **深度学习模型已升级为 MLP v2**：Swish激活+残差块(1层)+FeatureInteraction(64d)+OneCycleLR+标签平滑(0.03)+梯度裁剪(3.0)，AUC=0.709追平XGBoost，Brier=0.0799优于所有树模型。架构文件：`pipeline.py` 中的 `MLPClassifierV2` + `SklearnMLPWrapper`。v1 架构(`MLPClassifierV1`)保留作为回退选项。
- 当前投产版标准能力已包含：Platt 概率校准、Champion-Challenger 比较、显式基线模型 `baseline_logistic_regression`、Agent vs Baseline 的 T 集对比输出、配置化经济假设（成本/渠道乘数/产能约束）、以及 `optimize_collection_policy_tool` 用于在不重训模型时重跑催收策略。
